...
кандидатов даёт `git diff --name-only origin/main...HEAD`, а не диф PR. Правка задела
инструменты прогона (гейт `mutation-full`, `.claude/commands/review-pr.md`, шаг 4) — `make
mutation` без `files`. В `make check` прогон не входит: там он шёл бы на каждой правке.
...
Правки уходят так: `make check`, коммит «Правки по ревью: …», прогон мутаций, пуш, `R` + 1,
шаг 3. Если `R` уже 3, а править надо по комментарию владельца, — всё равно шаг 3.
