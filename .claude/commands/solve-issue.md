...
Номер в руках — назови им вкладку терминала. Заголовок кладётся в файл сессии, а отдаёт
его терминалу хук `~/.claude/hooks/warp-tab-title.sh`: сессия не меняет свой рабочий
каталог, и ветку задачи хук сам не увидит.

```bash
d=~/.claude/cache/warp-tab-title/active
[ -d "$d" ] && [ -n "${CLAUDE_CODE_SESSION_ID:-}" ] && printf '#%s' "<N>" > "$d/$CLAUDE_CODE_SESSION_ID"
```

Каталога нет — хук не установлен: пропусти шаг и не заводи каталог руками. Заголовок
вкладки к решению issue отношения не имеет и остановкой работы не становится.
...
5. Номер PR известен — перепиши им заголовок вкладки, тем же файлом, что в шаге 1:
   `printf '#%s · PR #%s' "<N>" "<PR>"`.
...
Удали заголовок вкладки: `rm -f ~/.claude/cache/warp-tab-title/active/"$CLAUDE_CODE_SESSION_ID"`.
Оставленный файл держит на вкладке номера уже влитой задачи до конца сессии.
