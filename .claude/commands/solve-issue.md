...
глобом. Правка задела инструменты прогона (гейт `mutation-full`, `.claude/commands/review-pr.md`,
шаг 4) — `make mutation` без `files`. В `make check` прогон не входит: там он шёл бы на каждой
правке.

Прогон оставляет запись — `reports/mutation/record.md`, формат в `docs/architecture/testing.md`,
«Запись прогона». После пуша опубликуй её в PR комментарием как есть, дописав в конец подпись из
`CLAUDE.md`: по ней ревьюер принимает твой прогон вместо своего (условия —
`.claude/skills/pr-light-check/SKILL.md`, «Запись прогона автора»). Первый прогон идёт до PR, и
его запись публикуется сразу после создания PR. Пока прогон идёт, ничего другого не запускай, как
ревьюер на `mutation-full`: под нагрузкой статус мутанта врёт в обе стороны
(`docs/architecture/testing.md`, «Таймауты и ошибки»), а твой прогон ревью переиспользует.
...
Правки уходят так: `make check`, коммит «Правки по ревью: …», прогон мутаций, пуш, запись
прогона в PR, `R` + 1, шаг 3. Если `R` уже 3, а править надо по комментарию владельца, — всё
равно шаг 3.
