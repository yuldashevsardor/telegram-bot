...
следующая сессия прочитает там код до мержа. Ненулевой код выхода с сообщением, что ветка на
`origin` осталась, — не провал уборки: дерево и локальная ветка уже удалены, повторять цель
неоткуда, а ветку и команду из вывода назови в отчёте.
