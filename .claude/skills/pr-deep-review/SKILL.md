...
**Баги** — гейт `bug-hunt`. Запусти встроенный скилл `code-review`, тот, что без префикса
плагина. Уровень: `high`, если в дифе есть TypeScript (гейт `smells`); иначе `medium` —
для дифа из одних bash-скриптов расширенный охват `high` даёт неуверенные находки и лишний
расход, а не баги. Добавь `--comment`, если этот флаг есть в аргументах: тогда находки
лягут inline-комментариями прямо в PR.

**Смеллы** — гейт `smells`. Запусти `mattpocock-skills:code-review`. Он ищет не баги,
а нарушения стандартов и смеллы по Фаулеру (Feature Envy, Speculative Generality,
Divergent Change и прочие) — то, чего первый скилл не находит. Вызывая, задай две вещи явно:
