...
шаг 3, гейт `docs`): ссылки на закрытые issue, выводимые перечни, дубли. Свой чек-лист
на них не заводи и повторно их не ищи: у проверки один владелец, и на PR из одной
документации, куда тебя не зовут вовсе, её выполняет он же.
...
git show origin/main:<изменённый файл> \
  | grep -oE '^export +(abstract +)?(class|interface|type|enum|const|function) +[A-Za-z_][A-Za-z0-9_]*'
git grep -l -w '<Символ>' origin/main -- ':(top)docs/architecture/'
...
ветке, а база PR видна всегда; `:(top)` обязателен, иначе из подкаталога `git grep` молча
вернёт пусто. Переименования спрашивай у `git`, а не у `gh`: в дифе GitHub метаданных
переименования может не быть, и `rename from` там не найдётся.

Кроме символов ищи путь `<каталог>/<файл>.ts` — доки называют и файлы, — а при
переименовании ещё и старое имя с его путём: устаревший абзац называет именно их.
...
- [question] docs/architecture/<файл>.md:65 — описывает <символ>, который PR меняет; не устарело ли
