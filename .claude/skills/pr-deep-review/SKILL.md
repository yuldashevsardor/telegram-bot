...
Гейт есть → ищи смеллы по Фаулеру (Feature Envy, Speculative Generality, Divergent Change
и прочие) сам. `mattpocock-skills:code-review` для этого не годится: он считает диф как
`git diff <фиксированная точка>...HEAD`, вторая сторона сравнения у него прибита к `HEAD`
рабочего дерева, а ветку PR ты не чекаутишь (шаг 3). Из основного дерева на `main` он
получит пустой диф, из worktree соседней задачи — чужую ветку, и её смеллы молча лягут
в вердикт как находки по этому PR.
...
Кавычки обязательны: без них шаблон раскроет шелл, и в zsh команда упадёт с
`no matches found` ещё до гита. `*` в pathspec гита проходит и через `/`, поэтому эти
три шаблона берут `.ts` на любой глубине.
