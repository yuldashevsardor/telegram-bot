...
`docs/architecture/testing.md`, «Порог». Прежде чем запускать цель, проверь запись прогона автора
...
`Final mutation score` нет. Какое из двух сообщений о падении что значит — в
`docs/architecture/testing.md`, «Чекер типов». `crashed with exit code null` — сбой машины, а не
PR: повтори цель один раз. Оборвался и повтор — гейт `n-a` с причиной «прогон оборван падением
чекера», вердикт BLOCKED: судить о мутантах не по чему. `ran out of memory` — `fail`: чекеру не
хватает лимита кучи из `checkerNodeArgs` на дереве этого PR.
...
(`docs/architecture/testing.md`, «Таймауты и ошибки»).
...
`docs/architecture/testing.md`, «Запись прогона». Проверь её перед запуском цели: на
`mutation-full` — сразу, на `mutation` — когда область собрана и не пуста. Последняя запись в PR
(ссылка на комментарий и маркер; пусто — записи нет) и head PR:

```bash
gh pr view <N> --json comments -q '[.comments[] | select(.body | contains("<!-- mutation-record "))] | last // empty | .url, (.body | capture("(?<m><!-- mutation-record [^>]*-->)").m)'
gh pr view <N> --json headRefOid -q .headRefOid
```

Запись принимается, если выполнено всё:

1. `head=` маркера совпадает с head PR;
2. `clean=yes`;
3. прогон дошёл до отчёта (`score=` не `none`) и покрыл область: на `mutation-full` — `scope=full`,
   на `mutation` — `scope=full` или пустой вывод команды ниже;
4. гейта `rebuild` нет: прогон автора мог идти на старом образе.

```bash
gh pr view <N> --json comments -q '[.comments[] | select(.body | contains("<!-- mutation-record "))] | last | .body | (split("\n")[] | select(test("^src/\\S+\\.ts$"))), (capture("\n- files: `(?<f>[^`]*)`").f | split(" ")[])' \
...
Путь в `files` засчитывается потому, что файла без единого мутанта в отчёте Stryker нет: файл из
одних типов, который автор назвал путём, в прогоне был, но мутированным не значится. Такой файл,
попавший в прогон автора только глобом, команда напечатает, и запись не принимается.
...
в дифе читаются вместе с причиной.

Не выполнено хоть одно условие — гоняй цель сам, как описано выше. Отказ от записи автора — не
находка ревью и на вердикт не влияет: ошибка процесса не должна стоить круга. Откуда прогон и
почему запись автора не принята, говорит строка `mutation:` вердикта (шаг 5).
...
публикуются — последней в PR остаётся запись гейта. С `--no-post` не публикуется и она.
...
(`docs/architecture/testing.md`, «Таймауты и ошибки»). Всю область повторять незачем: мутанты
...
требует ветку, влитую в `origin/main`, и удаляет её на origin, а ветка PR на ревью ещё живая.
...
mutation: ok/fail/n-a — <счёт из Final mutation score>, <весь src/ или файлы области> · прогон автора, <ссылка на запись> | свой прогон — <почему запись автора не принята> (n-a — причина)
