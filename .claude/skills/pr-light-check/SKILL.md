...
`docs/architecture/testing.md`, «Порог». Здесь только чтение вывода. Исход гейта — код выхода
цели, счёт — из строки `Final mutation score`. Выживших (`Survived`) и непокрытых (`NoCoverage`)
`clear-text` печатает выше, по одному: мутатор, `<файл>:<строка>:<колонка>` и замена. Их
перечисли в «Красном».
...
1. Первая команда даёт кандидатов: исходник — как есть, спеку — её зеркалом
   ...
   Хуки `mocha` (`*-hook.ts`) никто не импортирует, и области они не дают.
...
`n-a`, а не `ok`: в области нашлись только файлы без кода, и зелёный выход ничего не проверил.
...
`mutation-full` сначала сверь повторным прогоном — `make mutation files="<файл с выжившими>"`,
по одному файлу за раз: у порога 100 запаса нет, а на загруженной машине статус мутанта врёт в
обе стороны (`docs/architecture/testing.md`, «Таймауты и ошибки»). Всю область повторять
незачем: мутанты других файлов на статус этих не влияют. `clear-text` печатает только
выживших, поэтому статус и причину прежнего выжившего в повторе бери из отчёта. Мутатор, строка
и колонка — из первого прогона:

```bash
grep -oE '"mutatorName":"<мутатор>","replacement":"(\\.|[^"\\])*",("statusReason":"(\\.|[^"\\])*",)?"status":"[A-Za-z]+"[^}]*\},"start":\{"column":<колонка>,"line":<строка>\}' reports/mutation/mutation.html
```

Отчёт повтора содержит только его файл, но одна позиция — ещё не один мутант:
`ConditionalExpression` ставит `true` и `false` в одно место, а у `a && b` начало общее у всего
выражения и у `a`, и `true` там стоит дважды — с разным `end`. Совпадений несколько — твой тот,
чья `replacement` на месте от `start` до `end` даёт строку `+` под выжившим в первом прогоне.
Статусы остальных к делу не относятся.

- `Survived` — красное.
- `Killed`, и в `statusReason` сообщение проверки спеки — дрейф: гейт `ok`, а мутанта назови в
  «Красном» с пометкой «дрейф».
- `Timeout` или `Killed` с `Timeout of …ms exceeded` — повтор ничего не доказал: нагрузка прячет
  выжившего именно так. Гейт `fail`, а в «Красном» напиши, что мутанта нужно перепроверить на
  свободной машине.
