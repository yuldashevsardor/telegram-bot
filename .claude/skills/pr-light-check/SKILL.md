...
git grep -l 'from "test/<путь хелпера без .ts>"' -- 'test/*.spec.ts'
...
1. Первая команда даёт кандидатов: исходник — как есть, спеку — её зеркалом
   ...
   `test/`, который не спека, — хелпер: второй командой найди спеки, которые его импортируют, и
   возьми их зеркала. Хуки `mocha` (`*-hook.ts`) спеки не импортируют, и области они не дают.
...
у порога 100 запаса нет, а на загруженной машине статус мутанта врёт в обе стороны
(`docs/architecture/testing.md`, «Таймауты и ошибки»). Файлов с выжившими хватает, повторять
всю область незачем: мутанты других файлов на них не влияют. `clear-text` печатает только
