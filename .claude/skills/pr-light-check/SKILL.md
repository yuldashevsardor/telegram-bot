...
```bash
gh pr view <N> --json comments -q '[.comments[] | select(.body | test("^<!-- mutation-record "))] | last | .body | (split("\n")[] | select(test("^src/\\S+\\.ts$"))), (capture("\n- files: `(?<f>[^`]*)`").f | split(" ")[])' \
...
даёт ссылку, а не называет автора: запись ревьюера прошлого круга лежит в том же треде и на том же
head принимается наравне с авторской, а по комментарию автора от неё не отличить — оба аккаунта
один.
...
mutation: ok/fail/n-a — <счёт из Final mutation score>, <весь src/ или файлы области> · принятая запись, <ссылка> | свой прогон — <почему запись не принята> (n-a — причина)
