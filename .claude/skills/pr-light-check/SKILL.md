...
Обе запускают `tsc`, но по разным тиконфигам, и одна другую не подменяет. Скрипт `build`
(`package.json`) идёт по `tsconfig.json`, чей `include` — только `src`; `typecheck` —
по `tsconfig.check.json`, где к нему добавлены `test/**` и `migrations/**`. Зелёный `build`
на PR, который трогает спеки или миграции, про их типы не говорит ничего.
