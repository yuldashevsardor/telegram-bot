...
- `test` — те же спеки, что `coverage`, но без порога покрытия
  ...
  её зелёным, хотя `make check` у автора падает. Поэтому гейт `test` гоняет `make coverage`.
  Отчёт та пишет в `./coverage` дерева PR — каталог в `.gitignore`, диф от этого не меняется.
