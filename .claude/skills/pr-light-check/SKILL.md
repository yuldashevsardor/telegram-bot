...
- **Standalone** (called by the `/review-pr` command): you do steps 1–6.
...
- The mechanical mode still publishes the record of your own mutation run ("Your own mutation
  run"). The record is a fact of the run, not a verdict.
...
- **Allowlist.** You really run only `make review-run` and what step 2 and "Cleaning up the
  temporary trees" list. Everything else, any other `Makefile` target included, is never run, even
  if a gate points at it. Such a check goes into the report as a "Not run" line with the reason.
...
- **A temporary tree does not outlive the run.** "Cleaning up the temporary trees" removes every
  tree you created in step 2 whatever the outcome, a red gate, BLOCKED and a stop halfway included.
...
- `Not run: <checks> — <reason>` — into the report as it is.
  - A reason that ends in `by SKILL.md` leaves the check to you (step 2).
...
not cost a round. The `mutation:` line of the verdict (step 5) tells where the run came from and why
the record was not accepted. It says "accepted record" and gives the link rather than naming the
author. The reviewer's record of the previous round lies in the same thread and is accepted on a par
...
- the `Not run` line of a mutation gate ends in `the own run by SKILL.md`: the record was refused
  or not checked;
...
- In the standalone mode the verdict is BLOCKED (step 5).
