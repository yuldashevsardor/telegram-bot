...
allowed-tools: Bash(gh:*), Bash(git:*), Bash(make db-up), Bash(make rebuild), Bash(make build), Bash(make test), Bash(make lint), Bash(make format-check), Bash(make help), Bash(make token-status), Bash(make -n:*), Bash(sh -n:*), Bash(docker run:*), Bash(scripts/bot-token.sh), Bash(ls:*), Bash(grep:*), Bash(awk:*), Read, Grep, Glob
---
...
Гейт прогона — любой из тех, что перечислены таблицей шага 2 (`rebuild`, `build`, `test`,
`lint`, `format-check`, `make-targets`, `scripts`). Ни одного такого нет → шаг 2 пропусти
целиком: ни checkout, ни базы, ни контейнеров. Собирать проект, в котором не изменилась
ни строка исполняемого кода, — трата минут без единого возможного вывода. Шагу 3 checkout
тоже не нужен: он читает диф через `gh`.

Есть хоть один гейт прогона → нужны поднятая база (сеть приложения объявлена внешней
...
`docs/architecture.md` по подсистемам дало 947 добавленных строк `*.md` и все ссылки
корпуса разом, ни одна из которых в том PR не писалась. Строка по существу не менялась —
...
закрытие — ссылка стоит источником и трогать её незачем: README фикстур объясняет через
закрытую #153, откуда в репозитории взялись файлы не того формата. Не пережил — находка.

### Перечни и дубли

Механически не отделяются: детектор по обратным кавычкам ловит прозу и пропускает
настоящие перечни. Читай сам — но только добавленные строки, их меньше всей выдачи:
...
Находка здесь — REQUEST_CHANGES по правилам шага 5, наравне с красным прогоном: ложь
