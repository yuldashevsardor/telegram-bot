...
# Горячий файл конфигурации смонтирован в контейнер по имени (docker-compose.app.yml), а
# bind-mount несуществующего пути Docker создаёт каталогом от root: приложение упало бы на чтении
# конфигурации, а удалять такой каталог пришлось бы из-под sudo. Поэтому файл появляется перед
# любым запуском compose, и пустой файл ничего не меняет — значения берутся из окружения.
# Проверка существования, а не безусловный touch: тот обновлял бы время правки на каждой цели, и
# работающий бот пересобирал бы конфигурацию на каждый make logs, а на каталоге от root падал бы
# «Permission denied», унося с собой и цель, которой этот каталог не мешает.
...
# Каталог создаём на хосте заранее (и в check, который гоняет тот же скрипт): несуществующий
# путь Docker создал бы сам, но от root, а в контейнере работает пользователь node — писать
# в такой каталог он бы не смог.
coverage: ## Тесты с покрытием, падают ниже порога; таблица в терминал, lcov — в ./coverage
	@mkdir -p coverage
...
# нагрузкой (docs/architecture/testing.md, "Timeouts and errors"). Поэтому прогон идёт под
...
# mutation, поэтому зелёный check ещё не значит, что ревью будет зелёным.
...
worktree-cleanup: ## Убрать это дерево задачи после влития PR (приложение, дерево, ветка) и подтянуть main в основном дереве
