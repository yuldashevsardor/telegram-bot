...
нужна. Саму `origin/main` цель поэтому обновляет отдельной командой: уборке ссылка не
нужна, но от неё ответвляют дерево задачи, а хук старта сессии сверяет по ней `.claude`
([`docs/architecture/testing.md`](docs/architecture/testing.md)). Стоит команда до проверки
слитости, поэтому ссылка обновляется и тогда, когда проверка уборку обрывает, — то есть после
squash-мержа. Остальные отказы — незакоммиченные изменения, недоступный origin и всё прочее, на
чём цель отказывает раньше, — ссылку не обновляют: команда стоит за ними. Уборку она не роняет,
но свой отказ называет: занятую соседом ссылку обновит следующая уборка, а повторяющийся отказ
значит `.lock`, оставшийся от убитого git, — его удаляют руками. Если сосед влил свой PR, пока
