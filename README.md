...
- `config/runtime.env` (шаблон `config/runtime.env.dist`) — единственный файл, который
  приложение перечитывает на ходу: его правка пересобирает конфигурацию, а его значения
  перекрывают `.env` и окружение контейнера. Файла может не быть вовсе. Что при этом
  меняется в работающем приложении, а что нет —
  [`docs/architecture/config.md`](docs/architecture/config.md), «Подписка на изменения».
