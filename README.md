...
попасть в ссылку проверки слитости, и цель отказывалась убирать уже влитую ветку. Стоит
команда до проверки слитости, поэтому ссылка обновляется и тогда, когда проверка уборку
обрывает, — то есть после squash-мержа. Остальные отказы — незакоммиченные изменения,
недоступный origin и всё прочее, на чём цель отказывает раньше, — ссылку не обновляют: команда
стоит за ними. Уборку она не роняет, но свой отказ называет: занятую соседом ссылку обновит
следующая уборка, а повторяющийся отказ значит `.lock`, оставшийся от убитого git, — его
удаляют руками. Свежее коммита, полученного `fetch` в начале уборки, `origin/main` не станет,
а чужой `fetch`, успевший записать в неё мерж соседа, копия отодвинет назад. Так же выглядит и
влитый соседом PR, пока уборка шла: `origin/main` остаётся на мерж старее подтянутого `main` —
`git status -sb` в основном дереве показывает `main` впереди неё, `[ahead N]`, где N — мерж
соседа вместе с коммитами его ветки, — а дерево, заведённое от `origin/main` командой из
`CLAUDE.md`, ответвится от той же старой точки. И то и другое — до первого `git fetch`.
