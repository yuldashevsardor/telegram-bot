...
и `git merge --ff-only origin/main`: двигать его больше некому, а сессия стартует именно там
и до создания своего дерева читает оттуда код и доки, то есть текстом до мержа. Основное
дерево не на `main`, отказал `fetch` или ff-мерж (свои коммиты в `main`, незакоммиченные
правки в тех же файлах) — цель говорит, что `main` не подтянут, и называет, чем подтянуть
его руками. Уборка дерева при этом уже сделана, а правки основного дерева не тронуты.
