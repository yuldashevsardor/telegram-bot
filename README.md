...
```bash
make token-add token=<токен от @BotFather>   # печатает только номер слота, сам токен — нет
```
...
Токен попадает в историю шелла — если это нежелательно, впишите его в `tmp/bot/tokens` руками,
последней строкой.
