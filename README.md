...
остался файл, который не удаляется (`coverage` и `reports` монтируются в контейнер, и
созданное там принадлежит чужому uid). На этом шаге цель и останавливается: при живом дереве
удаление его ветки и подтягивание `main` обещали бы убранное. В выводе сказано, что приложение
вместе с образом и томом уже погашено, а дальше — одно из двух. Дерево уцелело: устраните
причину и повторите цель из него же, повторное гашение уже погашенного приложения ничего не
портит. Дерево снято с регистрации, но каталог остался с неудалённым остатком — повторять цель
неоткуда, и вывод называет команды, которыми доделать уборку руками.

Отказ удаления ветки — локальной или на `origin`, например прав не хватило — уборку не
обрывает: всё необратимое к этому моменту сделано. Цель договаривает вывод, называет
команду для ручного удаления оставшейся ветки из основного дерева и завершается ненулевым
кодом: им этот исход и отличается от полной уборки, а `make` допечатает поверх свою строку
об ошибке. Дерево при этом уже убрано, повторять цель неоткуда.

Дойдя до конца, цель подтягивает `main` в основном дереве — свежим `git fetch origin main`
и `git merge --ff-only origin/main`: двигать его больше некому, а сессия стартует именно
там и до создания своего дерева читает оттуда код и доки, то есть текстом до мержа.
Основное дерево не на `main`, ff-мерж отказал (свои коммиты в `main`, незакоммиченные
правки в тех же файлах) или не прошёл `fetch` — цель говорит, что `main` не подтянут, и
называет, чем подтянуть его руками. Уборка дерева при этом уже сделана, а правки
основного дерева не тронуты.
