...
то есть текстом до мержа. Ссылка своя, а не общая `origin/main`, потому что общую в этот же
момент может держать на запись `fetch` соседней сессии, и тогда подтягивание пропало бы
целиком из-за ссылки, которая ff-мержу не нужна. `origin/main` цель поэтому не обновляет —
её обновляет `fetch` проверки слитости в начале уборки. Если сосед влил свой PR, пока
уборка шла, `origin/main` так и остаётся на мерж старее подтянутого `main`:
`git status -sb` в основном дереве показывает `main` впереди неё — `[ahead N]`, где N —
мерж соседа вместе с коммитами его ветки, — а дерево, заведённое от `origin/main` командой
из `CLAUDE.md`, ответвится от той же старой точки. И то и другое — до первого `git fetch`.
