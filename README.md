...
чтобы начать с чистой базы — `make db-reset` (гасит базу и стирает её данные; в дереве задачи
`tmp/pgsql` — симлинк, поэтому цель чистит данные в основном дереве, а сам симлинк оставляет).
...
| `make db-reset`                  | погасить базу и стереть её данные                |
