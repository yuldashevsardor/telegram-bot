...
Пути таких деревьев цель печатает — погасите их там `make app-down` (или дождитесь конца разовой
команды) и повторите. Проверка best-effort: контейнер чужого дерева, исчерпавший
`restart: on-failure`, в неё уже не попадает, хотя сессия там продолжается, — она страхует от
случайного сброса, а не заменяет договорённость между сессиями.
