...
промежуток уборки. Отодвинуть ссылку назад она может шире первой: между хвостовым `fetch` и ею
стоит ff-мерж, и чужой `fetch`, успевший в этот промежуток записать мерж новее, копия
перепишет. Её отказ молчит: сказать он может только о ссылке, занятой соседом между двумя
копиями, — залипший `.lock` отказал бы и первой, с её сообщением. Если вторая копия отказала,
`origin/main` остаётся на коммите начала уборки: дерево, заведённое от неё командой из
`CLAUDE.md`, ответвится от той же точки, а `git status -sb` в основном дереве показывает
`main` впереди неё — `[ahead N]`, где N — то, что влили соседи, пока уборка шла. Не влили
ничего — `main` и `origin/main` стоят на одном коммите, и `[ahead N]` не печатается. И то и
другое — до первого `git fetch`.
