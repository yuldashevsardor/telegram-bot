...
работать, пока подняты контейнеры приложения других деревьев: иначе чужая сессия теряет базу
прямо посреди работы — её контейнер уходит в перезапуск на упавших миграциях, а данные уже не
вернуть. Пути таких деревьев цель печатает; погасите их там `make app-down` и повторите.
