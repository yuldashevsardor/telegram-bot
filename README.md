...
нужна. Саму `origin/main` цель поэтому обновляет отдельной командой, чей отказ ни на что
не влияет: уборке она не нужна, но от неё ответвляют дерево задачи, а хук старта сессии
сверяет по ней `.claude` (`scripts/claude-worktree-guard.sh`). Если сосед влил свой PR,
пока уборка шла, `origin/main` так и остаётся на мерж старее подтянутого `main`:
