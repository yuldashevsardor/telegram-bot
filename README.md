...
- `.runtime.env` — единственный файл, который приложение перечитывает на ходу: его правка
  пересобирает конфигурацию, а его значения перекрывают `.env` и окружение контейнера. Файл
  создают цели `make` и `scripts/worktree-init.sh`, пустой он ничего не меняет. В контейнер он
  смонтирован по имени, поэтому правка «на месте» доезжает, а сохранение через временный файл с
  переименованием (`vim` по умолчанию) — нет. Что при этом меняется в работающем приложении, а
  что нет — [`docs/architecture/config.md`](docs/architecture/config.md), «Подписка на
  изменения».
