...
там уже не было». Цель договаривает вывод, называет команду, которой с веткой разбираются
вручную из основного дерева, и завершается ненулевым кодом: им эти исходы и отличаются от
полной уборки, а `make` допечатает поверх свою строку об ошибке. Дерево при этом уже
убрано, повторять цель неоткуда.
