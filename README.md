...
`git status -sb` в основном дереве печатает `## main...origin/main [ahead 1]`, а дерево,
заведённое от `origin/main` командой из `CLAUDE.md`, ответвится от той же старой точки.
И то и другое — до первого `git fetch`.
