...
уже показывала влитой. Свежее коммита, привезённого тем `fetch`, ссылка при этом не станет, а
чужой `fetch`, успевший записать в неё мерж новее между двумя соседними строками, копия
отодвинет назад. Стоит команда до проверки слитости, поэтому ссылка обновляется и тогда, когда
проверка уборку обрывает, — то есть после squash-мержа. Остальные отказы — незакоммиченные
изменения, недоступный origin и всё прочее, на чём цель отказывает раньше, — ссылку не
обновляют: команда стоит за ними. Уборку она не роняет, но свой отказ называет: занятую
соседом ссылку обновит следующая уборка, а повторяющийся отказ значит `.lock`, оставшийся от
убитого git, — его удаляют руками.

Копия повторяется в конце, следом за подтянутым `main`, и сети ей уже не нужно: первая
осталась на коммите начала уборки, а свежую ссылку к этому моменту привёз `fetch`
подтягивания. Без второй `origin/main` отставала бы от `main` основного дерева на весь
промежуток уборки. Её отказ молчит: сказать он может только о ссылке, занятой соседом между
двумя копиями, — залипший `.lock` отказал бы и первой, с её сообщением. Если вторая копия
отказала, `main` оказывается впереди `origin/main`: `git status -sb` в основном дереве
показывает `[ahead N]`, где N — мерж этой ветки вместе с её коммитами и всё, что влили соседи,
а дерево, заведённое от `origin/main` командой из `CLAUDE.md`, ответвится от точки начала
уборки. И то и другое — до первого `git fetch`.
...
тронуты. До второй копии цель в этих исходах не доходит, и `origin/main` остаётся на коммите
начала уборки — вместе с не сдвинутым `main`.
