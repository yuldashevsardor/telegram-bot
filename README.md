...
(`scripts/claude-worktree-guard.sh`). Стоит команда до проверки слитости, поэтому ссылка
обновляется и тогда, когда проверка уборку обрывает, — то есть после squash-мержа. Отказы
до неё, незакоммиченные изменения и недоступный origin, ссылку не обновляют: команда стоит
за ними. Уборку она не роняет, но свой отказ называет: занятую соседом ссылку обновит
следующая уборка, а повторяющийся отказ значит `.lock`, оставшийся от убитого git, — его
удаляют руками. Если сосед влил свой PR, пока уборка шла, `origin/main` так и остаётся на
мерж старее подтянутого `main`:
