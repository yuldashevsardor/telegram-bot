...
`[ahead N]` не печатается. Отказал и мерж — `main` остался не подтянутым и позади ссылки.
Всё это — до первого `git fetch`, а залипший `.lock` не отпустит ссылку и ему: до удаления
файла руками она остаётся старой.
