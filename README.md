...
нужна. Саму `origin/main` цель поэтому обновляет отдельной командой, и делает это в самом
начале, до проверки слитости: уборке ссылка не нужна, но от неё ответвляют дерево задачи, а
хук старта сессии сверяет по ней `.claude` (`scripts/claude-worktree-guard.sh`), — поэтому
она обновляется и на тех отказах, которые уборку обрывают, включая squash-мерж. Уборку эта
команда не роняет, но её отказ называет: занятую соседом ссылку обновит следующая уборка, а
повторяющийся отказ значит `.lock`, оставшийся от убитого git, — его удаляют руками. Если
сосед влил свой PR, пока уборка шла, `origin/main` так и остаётся на мерж старее
подтянутого `main`:
