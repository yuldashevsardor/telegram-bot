...
Дойдя до конца, цель подтягивает `main` в основном дереве — свежим `git fetch` и ff-мержем
по нему: двигать `main` больше некому, а сессия стартует именно там и до создания своего
дерева читает оттуда код и доки, то есть текстом до мержа. Забирает этот `fetch` в свою
ссылку `refs/worktree-cleanup/main`, а не в общую `origin/main`: общую в этот же момент
может держать на запись `fetch` соседней сессии, и тогда подтягивание пропало бы целиком
из-за ссылки, которая ff-мержу не нужна. `origin/main` он поэтому не обновляет — её
обновляет `fetch` проверки слитости в начале уборки. Если сосед влил свой PR, пока уборка
шла, `origin/main` так и остаётся на мерж старее подтянутого `main`: дерево, заведённое от
неё командой из `CLAUDE.md`, ответвится от той же старой точки, пока в основном дереве не
сделан `git fetch`.
