...
Отказ `git push origin --delete` (сеть, права, ветку уже удалили) уборку не обрывает: всё
необратимое к этому моменту сделано. Цель договаривает вывод, называет команду для ручного
удаления ветки и завершается ненулевым кодом — им этот исход и отличается от полной уборки.
