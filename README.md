...
Отказ `git push origin --delete` — например, прав на удаление ветки не хватило — уборку не
обрывает: всё необратимое к этому моменту сделано. Цель договаривает вывод, называет
команду для ручного удаления ветки из основного дерева и завершается ненулевым кодом: им
этот исход и отличается от полной уборки, а `make` допечатает поверх свою строку об
ошибке. Дерево и локальная ветка при этом уже убраны, повторять цель неоткуда.
