...
работать, пока в сети базы есть контейнеры приложения других деревьев — и поднятый бот, и
одноразовый контейнер разовой цели вроде `make test`. Иначе чужая сессия теряет базу прямо
