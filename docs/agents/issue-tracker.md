...
- No new labels: the stock ones are enough (`gh label list`).
- The triage machine of the `triage` skill and its labels (`needs-triage` and the like) are not
  used. `docs/agents/triage-labels.md` is absent on purpose.
...
its child issues labelled `wayfinder:<type>`. The repository has no labels of this family yet:
they are created together with the first map.

A child ticket is attached to the map as a native sub-issue. A block is a native GitHub
dependency:
...
Both operations take the numeric database id, not the issue number. Neither `#number` nor
`node_id` works. The id is printed by:
