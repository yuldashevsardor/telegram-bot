...
```
setup()
 └─ setupModules()        TaskQueue, Runner (singleton) → setupBot()
...
- **`LimitResolver`** (`src/domain/task-queue/limit-resolver.ts`) — порт: лимит есть свойство ключа, а не отдельной задачи, поэтому очередь спрашивает его один раз, когда заводит партицию. Реализация для бота — `TelegramLimitResolver` (`src/infrastructure/bot/telegram-limit-resolver.ts`): отрицательный chat ID означает группу (`isGroupChat`), остальное — приватный лимит.
...
- **`test/domain/task-queue/`** — два spec-файла подсистемы §6, оба без моков: `rate-limit.spec.ts` проверяет `RateLimit` (свежий свободен → занят после `reserve()` → снова свободен после остывания → повторный `reserve()` бросает `RateLimitIsBusy`), `partition.spec.ts` — `Partition` (приоритет внутри ключа, FIFO внутри приоритета, резервация в `take()`, и что партиция пуста, но не `isIdle()`, пока идёт остывание). Ожидание остывания — `await` по промису, а не голый колбэк `setTimeout`, поэтому Mocha действительно дожидается ассерции.
...
- **Итог:** реального тестового покрытия практически ноль. Ни у доменных сервисов (`TaskQueue`, `Runner`, `FontConvertor`, `UserService`), ни у инфраструктуры (`Bot`, middleware, `Config`, DI-контейнер), ни у хелперов тестов нет.
