...
говорит «это разметка», а не «это шрифт». Корневой тег она поэтому и не ищет: документ
вправе открываться объявлением XML, доктайпом, комментарием или инструкцией обработки, и
перечислять эти прологи значило бы дописывать сигнатуру на каждый следующий. Вместо
перечисления сигнатура описана классом байт: за `<` идёт буква корневого тега или `!`
доктайпа и комментария, а дальше — текст. Текст в ней не для красоты: начала разметки
