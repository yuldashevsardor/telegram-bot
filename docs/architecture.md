...
- **Конфигурация прогона — `.mocharc.json`** в корне: `require` (`ts-node/register/transpile-only`, `tsconfig-paths/register`), маска `spec` и `color`. Скрипт `test` в `package.json` сведён к голому `mocha`. `tsconfig-paths/register` обязателен: без него `ts-node` не разрешает алиас `app/*` и mocha падает на загрузке spec-файла.
- **`transpile-only` — вынужденная мера, а не выбор.** Mocha 9 грузит spec-файлы через `import()`, и на этом пути ts-node 10.9.2 отдаёт компилятору не тот исходник: под `strict` он сыплет `TS7006` на явно типизированные параметры (`function delay(ms: number)` — «Parameter 'ms' implicitly has an 'any' type»). Тот же файл, загруженный обычным `require()`, компилируется без ошибок. Типы тестов поэтому проверяет не прогон, а `npm run typecheck` (§15). Обходной путь уходит вместе со сменой раннера — issue [#69](https://github.com/yuldashevsardor/telegram-bot/issues/69).
