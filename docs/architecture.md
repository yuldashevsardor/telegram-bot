...
`.otf` коллекция отвергается здесь, до работы выбранной пары и одинаково для всех пар.
...
документ, и файл с отступом перед ним движок не открывает. Перед любой другой разметкой —
BOM и ведущие пробельные символы; по той же причине `?` не входит в класс начала
разметки, иначе отступ стал бы допустим и перед объявлением. Отступ ограничен пределом,
