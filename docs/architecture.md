...
надвое молча — запись при этом выглядит целой. Тип значения на ключ не влияет:
`serialize-error` заворачивает не-`Error` в `NonError`, так что развилки «Error под `cause`,
остальное под `error`» нет ни в фабриках, ни в вызовах логгера.
