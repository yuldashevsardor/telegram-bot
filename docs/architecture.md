...
Проверки разные — первые байты головы против разбора каталога таблиц, — но набор версий у
них один: `SFNT_VERSIONS` в `font-convertor/sfnt-version.ts`, откуда сигнатура берёт
байты, а кодек читает те же значения числом. Двумя списками этому набору быть нельзя:
расхождение ломает не сборку, а поведение — версия, известная только сигнатуре, доедет до
кодека и упадёт там `InvalidSfnt`, известная только кодеку — не пройдёт вход.
...
вправе стоять доктайп, комментарий или инструкция обработки, и перечислять эти прологи
значило бы дописывать сигнатуру на каждый следующий. Вместо перечисления сигнатура одна и
описана классом байт: за `<` идёт буква корневого тега, `!` доктайпа и комментария или `?`
инструкции обработки, а дальше — текст. Текст в классе не для красоты: начала разметки
одного мало, потому что заголовок EOT открывается размером файла и у фикстуры его младшие
байты складываются в `<m` — двоичная голова прошла бы как документ.

Границы «движок это откроет» вход не держит, и не по бедности языка сигнатур, а потому
что это не его вопрос. Объявление XML обязано открывать документ, поэтому файл с отступом
перед `<?xml` движок не откроет — но содержимое от отступа не изменилось: это тот же
шрифт, только со сломанной разметкой. Отвергнув его здесь, домен ответил бы
`InvalidFontSignature` — «расширение не сошлось с содержимым» — про файл, у которого оно
сошлось. О сломанном документе сообщает тот, кто документ разбирает, и его ошибка
пользователю точнее. По той же причине проверку проходит одинокий комментарий: разметка —
да, шрифт — нет, и выясняет это движок.

Смещение сигнатуры фиксировано не у всех: у двоичных форматов оно жёсткое, а у SVG перед
разметкой законно стоит префикс, и сигнатура сама объявляет, какой префикс перед ней
пропускается — UTF-8 BOM и ведущие пробельные символы. Отступ ограничен пределом, иначе
голова файла росла бы вместе с ним, и предел этот считается за BOM, а не вместе с ним:
общий бюджет означал бы, что невидимый BOM укорачивает допустимый отступ. Предельная
