...
- **`Planner`** (`src/domain/planner/planner.ts`) держит три приоритетные корзины и, на каждый чат, лениво создаваемый `SlotManager` по ключу `chatId` (`limits.group` или `limits.private` в зависимости от `message.isGroup`), плюс один общий `commonManager` (`limits.common`). Поскольку он ищет первое *подходящее* сообщение, а не строго голову очереди, порядок доставки не является строгим FIFO — сообщение зажатого рейт-лимитом чата может быть пропущено в пользу более позднего сообщения в другой чат. `Planner.managers` чистится по факту разгрузки очереди: как только `pull()` видит, что все три корзины пусты, из `Map` удаляются менеджеры без активной резервации (issue [#32](https://github.com/yuldashevsardor/telegram-bot/issues/32); до этого из неё не удалялось ничего, и каждый chat ID, с которым бот когда-либо общался, оставался в памяти на всё время жизни процесса). Свободный менеджер неотличим от только что созданного, поэтому его удаление лимитов не меняет; занятый переживает уборку, и его заберёт следующая — таймеров и TTL в уборке нет. Число живых менеджеров печатает тот же периодический лог, что и остаток очереди.
