...
- **`test/services/message-broker/slot-manager/slot-manager.spec.ts`** — **единственный** spec-файл во всём репозитории. Он напрямую создаёт `SlotManager` (без моков) и проверяет: свежий менеджер свободен → не свободен после `reserve()` → снова свободен после истечения интервала → повторный `reserve()` бросает исключение. Проверка «снова свободен после таймаута» завёрнута в голый колбэк `setTimeout` без `done()`/await, поэтому Mocha её не дожидается.
- **Сейчас `npm test` красный.** До добавления `tsconfig-paths/register` в скрипт `test` (§15) прогон вообще не стартовал: spec импортирует `src/domain/slot-manager/slot-manager` относительно `baseUrl`, а `ts-node` такие пути не разрешает — mocha падала на загрузке файла. Теперь тесты действительно исполняются и обнажают два бага в самом spec-файле: последняя проверка написана как `expect(manager.reserve.call(manager)).to.throw(...)` (вызов происходит внутри `expect`, исключение улетает мимо ассерции), а отложенная ассерция из `setTimeout` срабатывает уже после завершения прогона и роняет процесс необработанной `AssertionError`. Оба бага живут в тесте, а не в `SlotManager`.
...
- `DATABASE_URL` в `.env.dist` собирается, но самим приложением не потребляется — только `node-pg-migrate` (§12).
- Похожее на настоящее значение `BOT_TOKEN` остаётся в истории git, хотя из `.env.dist` уже убрано (§12).
...
- **Сборка `npm run build` даёт неработоспособный `build/`**: `tsc` не копирует `.ftl`-локали, а `Bot.setupFlavor` ищет их по `<cwd>/src/infrastructure/bot`, поэтому `npm start` падает с `ENOENT: scandir '.../src/infrastructure/bot'`. В дев-среде это не проявляется (там `ts-node` и `src/` на месте), но любой сценарий запуска из `build/` сломан (issue #19).
