...
документа без объявления — BOM и ведущие пробельные символы, не длиннее предела: иначе
голова файла должна была бы расти вместе с отступом. Предел префикса заложен и в
`headLength` — пропущенные байты сокращают полезную часть головы.
