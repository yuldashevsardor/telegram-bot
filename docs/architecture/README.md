...
Из вывода вычтены три каталога роли `shared/` (имя у них своё) и `font-forge/`; дерево
правилу отвечает целиком, и вывод пуст. Прячущие каталоги без спутников команда не
проверяет.
