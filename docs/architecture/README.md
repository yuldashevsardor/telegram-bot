...
каталоге), и слово каталога, которое путь над ним уже назвал. Второе — только если в имени
каталога остаётся слово, которого путь не называл: каталог из одних названных слов
(`convertor/` в `font-convertor/`) остановился на последнем и правилу отвечает. Из вывода
...
find src -name '*.types.ts' -o -name '*.errors.ts' | while read -r f; do m="${f%.*.ts}"; \
    ...
    [ "$rep" != "$own" ] && echo "$rep" | grep . | while read -r w; do \
