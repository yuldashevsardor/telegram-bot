...
`.ftl`; собирает их обход всей подсистемы (`createFluent(__dirname)` — `bot.ts:173`,
`locale.ts:40`), так что в новом каталоге `.ftl` не окажется ни одного, а имя будет то же.
...
пар сейчас нет). Старшинство у «прячет»: `convertor/` держит каталоги братьев `<from>/`, но
снаружи из него импортируют только `convertor-factory.ts`, и в пути спеки он не отражается,
а `command/`, `filter/`, `middleware/` и `conversation/` отдают наружу и базовый класс, и
братьев, и отражаются. Расхождения печатает команда, и в её выводе — только спеки каталогов
с префиксным именем:
