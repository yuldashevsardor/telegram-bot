...
из трёх оснований — прячет, собирает, держит главный со спутниками, — иначе не заводится.
...
главный: в `shared/` спутники уезжали в каталог всегда.
...
на два модуля подсистемы). Один каталог правилу не отвечает: в `telegram/session/` лежат
три файла разных ролей (`pgsql-storage.ts`, `session.helper.ts`, `session.types.ts`), и ни
один не прячет остальных.
...
Путь спеки повторяет путь исходника, но каталог, заведённый вокруг одного главного файла
подсистемы — прячущий его внутренности или держащий его со спутниками, — в пути спеки не
отражается: файлы `convertor/`, `eot-packer/`, `font-forge/` и `font-signature-matcher/`
проверяют спеки прямо из `test/font-convertor/`. Каталог модуля, адаптера и роли
(`platform/request-context/`, `bootstrap/config/parser/`, `shared/fs/`), каталог братьев и
каталог каждого брата (`telegram/command/start/`, `telegram/middleware/mutation/`) путь
спеки повторяют. Расхождения печатает команда — по строке на спеку, и других расхождений,
кроме таких каталогов, быть не должно:

```bash
find test -name '*.spec.ts' | while read -r s; do base=$(basename "$s" .spec.ts); \
    src=$(find src -name "$base.ts" | head -1); [ -z "$src" ] && continue; \
    [ "$(dirname "$src" | sed 's#^src/##')" != "$(dirname "$s" | sed 's#^test/##')" ] \
    && echo "$s | $src"; done
```
