...
`bootstrap/config/config-container.ts`: найденное значение `get()` приводится к `ValueByPath`, и
...
исходе: собран ли контекст, `create()` решает по тому же значению, что и геттеры, иначе
признаков готовности было бы два и спека, сбросившая контекст, получила бы такой, который
`create()` не пересобирает, а геттеры не отдают. Значение одно на все три части (поле `parts`),
а не поле на часть: кладётся оно только после сборки всех, так что упавший на конфиге
`create()` оставляет контекст пустым, и следующий начинает с нуля, а контекста, собранного
наполовину, не допускает тип.
