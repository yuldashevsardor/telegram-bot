...
три `as`. Остались три: шаг обхода по точкам приводит текущий узел к
`UnknownObject`, найденное значение `get()` — к `ValueByPath`, и по той же причине к
`ValueByPath` приводится пара значений, которую `onChange()` отдаёт слушателю
([`config.md`](./config.md)) — обход компилятору не проследить.
...
уже `init()` контейнера, поэтому короткий участок между ним и подпиской логгера конфигурация
следит за файлом без канала для отказов — окно короче одного опроса
([`config.md`](./config.md)).
