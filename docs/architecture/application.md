...
1. `gracefulStop()` → `application.stop()` → `process.exit(0)`; ошибка в цепочке —
   `fail()` и код 1.
2. `Application.stop()` до `setup()` и после конца остановки ничего не делает. Источник
   конфигурации останавливается в начале `terminate()` (`cc.stop()`), до общего срока и вне
   него: по истечении срока `terminate()` возвращается, а оставленный опрос файла пересобирал
   бы конфигурацию уже закрытого приложения ([инвариант](./invariants.md)). Первый вызов
