...
логгера → подписка логгера на отказы пересборки и `watch()` на наблюдаемый файл. Наблюдение
включается последним: до подписки логгера отказ первой же пересборки было бы некуда написать
([`config.md`](./config.md)).
...
2. `application.setup()`:
   - `ApplicationContext.create()` — `dotenv.config()` в `ConfigEnvStorage.load()` один раз и
     ...
     конфигурации ([`config.md`](./config.md)). В конце — наблюдение за файлом: правка
     пересобирает значения, и подписчики её пути получают новое (в приложении их пока нет,
...
2. `Application.stop()` до `setup()` и после конца остановки ничего не делает. Наблюдение за
   файлом конфигурации гасится в начале `terminate()`, до общего срока и вне него: по
   истечении срока `terminate()` возвращается, а оставленный опрос пересобирал бы
