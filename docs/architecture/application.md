...
`RequestContext` лежат в ветке `Bootstrap` рядом с `ConfigContainer`, хотя их классы — в
`platform/`. Словарь лежит в `shared/`, а не в `bootstrap/container/`: иначе за именем
собственной зависимости домен ходил бы в корень сборки.
