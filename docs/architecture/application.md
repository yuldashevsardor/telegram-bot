...
Глобален не путь в объекте, а строка внутри `Symbol.for`; как она строится и чем кончается
совпадение — в шапке `shared/tokens.ts`. Поэтому `Tokens.Bootstrap.RequestContext` и
...
три `as`. Остались два, оба в `bootstrap/config/config-container.ts`:
найденное значение `get()` приводится к `ValueByPath`, и по той же причине к `ValueByPath`
приводится пара значений, которую `onChange()` отдаёт слушателю ([`config.md`](./config.md)), —
обход по точкам компилятору не проследить. Сам обход приведения не требует: он сужает тип
стражем.
...
`ConfigFileStorage` над `ConfigEnvStorage` и его `init()` → `RequestContext` → выбор адаптера
логгера → подписка логгера на отказы пересборки конфигурации. Само наблюдение за файлом завёл
уже `init()` контейнера, но окна без адресата это не создаёт: до подписки идут только синхронный
код и продолжение `await`, а колбэк наблюдателя — отдельная задача в очереди
([`config.md`](./config.md)).
...
сломалась бы молча. Поэтому повторный `create()` не ошибка, а та же сборка: `create()`
асинхронный (конфиг собирается через `init()`) и держит промис идущей сборки, так что вызов
посреди неё ждёт её, а не собирает второй контекст. После сборки промис забывается при любом
исходе: собран ли контекст, `create()` решает по тем же полям, что и геттеры, иначе признаков
готовности было бы два и спека, обнулившая поля, получила бы контекст, который `create()` не
пересобирает, а геттеры не отдают. Поля заполняются только после сборки всех частей: упавший
на конфиге `create()` оставляет контекст пустым, и следующий начинает с нуля.
...
`new` в `app.ts`, в контейнере не значится. Состояние цикла — одно поле `state` (варианты и
почему не флаги — тип `State` там же). Идущие `setup()` и `stop()` держат в нём свой промис,
и повторный вызов ждёт его, а не проходит шаг заново. Экземпляр одноразовый: после конца
остановки ни `setup()`, ни `run()`, ни `stop()` ничего не делают. Перезапустить его нечем —
контейнер после `close()` второй `setup()` не переживает (выше), а `ApplicationContext` один
на процесс.
...
2. `application.setup()`:
   - `ApplicationContext.create()` — `dotenv.config()` в `ConfigEnvStorage.load()` один раз и
     явно, поверх него значения наблюдаемого файла, дальше разбор и валидация всей
     конфигурации ([`config.md`](./config.md)). `init()` контейнера заодно заводит наблюдение
     за файлом: правка пересобирает значения, и подписчики её пути получают новое (в приложении
     их пока нет, [инвариант](./invariants.md)).
   ...
   - `container.get()` для `TaskQueue`, `Runner` и `Bot`: вместе с `Bot`, до его
     конструктора, инстанцируются все обработчики пайплайна ([`bot.md`](./bot.md)).
   ...
   Упавший `setup()` повторный вызов не повторяет, а отдаёт тот же отказ: процесс после
   отказа завершает `fail()`.
3. `application.run()`: `runner.run()` — синхронный, ставит цикл очереди на `setTimeout`
   ...
   `bot.run()` — long polling в фоне ([`bot.md`](./bot.md)). До конца `setup()` и на
   запущенном приложении — `RuntimeError`, после начатой остановки — ничего (ниже).
...
**Триггер:** первый `SIGINT`/`SIGTERM` (`process.once`, у каждого сигнала свой слушатель).
Повторный сигнал того же вида слушателя уже не находит: Node применяет действие по
умолчанию и убивает процесс (130/143 вместо `exit(0)`) посреди начатой остановки. Сигнал
другого вида снова зовёт `gracefulStop()`, но вторую остановку не запускает: его `stop()`
ждёт идущую (шаг 2), и оба `exit(0)` приходятся на её конец.

1. `gracefulStop()` → `application.stop()` → `process.exit(0)`; ошибка в цепочке —
   `fail()` и код 1.
2. `Application.stop()` до `setup()` и после конца остановки ничего не делает. Наблюдение за
   конфигурацией снимается в начале `terminate()` (`cc.unwatch()`), до общего срока и вне него:
   по истечении срока `terminate()` возвращается, а оставленный опрос файла пересобирал бы
   конфигурацию уже закрытого приложения ([инвариант](./invariants.md)). Первый вызов
   переводит приложение в остановку, а каждый следующий во время неё ждёт её же промис, после
   её отказа — отдаёт тот же отказ. Сама остановка ждёт `shutdown()` не дольше
   `GRACEFUL_SHUTDOWN_TIMEOUT`; по истечении пишет `warning` и возвращается, а брошенный шаг
   продолжает выполняться, пока его не оборвёт `process.exit(0)` шага 1. `withTimeout()`
   гасит отказ опоздавшего шага, иначе он всплыл бы `unhandledRejection` уже после остановки.
3. Сигнал посреди `setup()`: сперва `stop()` дожидается сборки `ApplicationContext` — вне
   общего срока, потому что и срок, и логгер остановки берутся из неё; упала сборка — `stop()`
   отдаёт её отказ. Затем `shutdown()` дожидается остальной настройки — уже в общем сроке, —
   и переходит к шагу 5. Бот не запущен: `run()`, который `bootstrap()` зовёт
   следом за настройкой, после начатой остановки ничего не делает и не бросает, иначе
   `bootstrap()` ушёл бы в `fail()` с кодом 1. Отказ самой настройки `stop()` отдаёт
   наружу: с одной ошибкой в `fail()` приходят и `gracefulStop()`, и `bootstrap()`, первый
   вызов синхронно завершает процесс — `critical` один, код 1.
4. `shutdown()`, если приложение запущено:
...
Общий срок обязан быть больше суммы двух частных (`ConfigValuesBuilder` проверяет) и меньше
