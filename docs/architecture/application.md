...
Конфигурация внедряется в конструктор декоратором параметра `@InjectConfig("limits.common")`
(`common/inject-config.ts`). Токен на всю конфигурацию один —
`Tokens.Infrastructure.ConfigValue`, а какое значение отдать, говорит путь: декоратор вешает
его тегом на параметр, биндинг `toDynamicValue` читает тег из запроса и разрешает путь в
`ConfigContainer`, взятом у `ApplicationContext` (ниже). Область биндинга — transient
(умолчание `toDynamicValue`): путь у каждого параметра свой, и `inSingletonScope` раздал бы
всем первое разрешённое значение.

Путь — строковый литерал, но не произвольный: его тип собирается из формы `ConfigContainer`
(`ConfigPaths`), поэтому опечатка в ключе, путь сквозь примитив (`"tempDir.nope"`) и
приватное поле конфига не компилируются, а редактор подсказывает доступные пути. Тип
параметра компилятор при этом не сверяет — у legacy-декоратора параметра нет связи с типом,
и объявить `@InjectConfig("limits.common") x: string` он не помешает; расхождение поймает
первый же резолв. Прежний `@ConfigValue` не проверял ни ключ, ни тип и падал не на старте,
а при первом обращении к свойству — то есть, возможно, сильно позже.

Декоратор ничего не резолвит сам: он только вешает метаданные, значение подставляет
inversify. Отсюда два следствия — модуль не импортирует `container` (цикл, который держал
`@ConfigValue`, не возникает; тип `ConfigContainer` берётся `import type` и стирается), и
класс с `@InjectConfig` строится в тесте обычным `new`
(`test/domain/task-queue/task-queue.spec.ts`).
...
значения из него раздаёт `@InjectConfig` (выше), причём берёт он их у самого контекста, а не
из этого биндинга. Контекст не инжектится никуда, иначе он стал бы вторым
