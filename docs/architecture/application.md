...
резолв «No matching bindings found»; обработчики пайплайна резолвятся вместе с `Bot`
(`BotHandlers`, [`bot.md`](./bot.md)), команды и разговоры — по своим реестрам (шаги 7-8),
поэтому символ без биндинга валит резолв `Bot`, то есть старт, а не первый апдейт;
middleware и фильтры заданы списками в `Container.resolveBotHandlers()` — забытый в
...
2. `application.setup()`:
   ...
   - `Database.check()` — первый резолв `Database`, то есть здесь же отрабатывает её
     ...
     первом апдейте.
   - `container.get()` для `TaskQueue`, `Runner` и `Bot`: вместе с `Bot` инстанцируются все
     обработчики пайплайна (`BotHandlers`, [`bot.md`](./bot.md)), до его конструктора;
     пустой `BOT_TOKEN` валит конструктор `Bot` уже после проверки базы
     ([`config.md`](./config.md)).
