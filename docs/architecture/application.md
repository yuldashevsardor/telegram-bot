...
три `as`. Остались два, оба в `ConfigContainer.get()`: шаг обхода по точкам приводит текущий
узел к `UnknownObject`, а найденное значение — к `ValueByPath`, потому что обход компилятору не
проследить.
...
1. `gracefulStop()` → `application.stop()` → `process.exit(0)`; ошибка в цепочке —
   `fail()` и код 1.
2. `Application.stop()` до `setup()` и после конца остановки ничего не делает. Первый вызов
