...
и повторный вызов ждёт его, а не проходит шаг заново.
...
1. `gracefulStop()` → `application.stop()` → `process.exit(0)`; ошибка в цепочке —
   `fail()` и код 1.
2. `Application.stop()` до `setup()` ничего не делает. Первый вызов переводит приложение в
   остановку, а каждый следующий — во время неё или после — ждёт её же промис. Сама
   остановка ждёт `shutdown()` не дольше `GRACEFUL_SHUTDOWN_TIMEOUT`; по истечении пишет
   `warning` и возвращается, а брошенный шаг продолжает выполняться, пока его не оборвёт
   `process.exit(0)` шага 1. `withTimeout()` гасит отказ опоздавшего шага,
