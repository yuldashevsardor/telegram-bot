...
[`bot.md`](./bot.md)), поэтому символ без биндинга валит `Bot.setup()`, то есть старт, а
не первый апдейт; middleware и фильтры заданы списками в `bot.ts` — забытый в списке не
попадёт в пайплайн и не скажет об этом ничего. Строка внутри `Symbol.for` — глобальный
ключ: одно и то же имя в разных реестрах даёт один и тот же символ: биндинг под уже
занятым именем валит резолв «Ambiguous match». Поэтому имена в реестрах не пересекаются —
`RequestContext` (контекст запроса, `Infrastructure`) и `RequestContextMiddleware` (его
middleware, `Modules`). У второго ключ реестра — `Modules.Bot.Middleware.RequestContext`,
а строка внутри `Symbol.for` длиннее: короткую уже занял контекст запроса.
