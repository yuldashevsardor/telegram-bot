...
Стоит она умолчанием параметра конструктора, а сам параметр помечен `@unmanaged()` —
«контейнер этот аргумент не заполняет»:
...
`@unmanaged()` здесь обязателен, и проверять его нужно на сборке, а не только в dev:
`tsx` метаданные параметров не эмитит, а `tsc` эмитит, и параметр без пометки inversify в
собранном коде счёл бы своей зависимостью ([инвариант](./invariants.md) про явные
`@inject`).

Путь — строковый литерал, но не произвольный: его тип собран из формы `ConfigContainer`
(`ConfigPaths`), а тип результата выводится из того же места (`ValueByPath`). Поэтому
...
значения из него берут прямо у контекста, мимо контейнера (выше). Контекст не инжектится никуда, иначе он стал бы вторым
