...
асинхронный (конфиг собирается через `init()`) и держит промис сборки, так что вызов посреди
неё ждёт её, а не собирает второй контекст. Поля заполняются только после сборки всех частей:
упавший на конфиге `create()` оставляет контекст пустым и забывает промис, и следующий
начинает с нуля.
