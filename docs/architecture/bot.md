...
сверху вниз: каждый шаг рассчитывает на то, что заполнили предыдущие. Порядок шагов
держит `Bot.setup()`, а порядок внутри шагов 1-2, 5, 7 и 8 — списки `BotHandlers`
(`bot.types.ts`): их собирает `Container.resolveBotHandlers()` (`container.ts`), а `Bot`
получает готовыми через `@inject(Tokens.Bot.Handlers)` и вешает как есть. Контейнера `Bot`
не знает: `telegram/` не импортирует корень сборки, который импортирует его самого.
