...
`ConfigFileStorage` снимает опрос и зовёт `stop()` того, кого обернул. Снимает он ровно своего
слушателя (`fs.unwatchFile` вторым аргументом): без него ушли бы все слушатели этого пути в
процессе, включая чужой экземпляр, следящий за тем же файлом.
...
Интервал читает `getTimerDelay` с нижней границей `0`, поэтому верхний предел — общий для всех
таймеров Node.

Пустое значение в файле («`KEY=`») источник отбрасывает: задать им пустоту всё равно нельзя —
`ConfigParser` считает пустую строку отсутствием и берёт умолчание, — а перекрой оно базовый
источник, обнуление строки уводило бы значение не к переменной окружения, откуда его брали до
правки, а к умолчанию кода. Поэтому «обнулить строку» и «удалить строку» в наблюдаемом файле
значат одно и то же.
...
источника. `stop()` её отменяет: проход, уже стоящий на чтении снимка, значения не подменит —
