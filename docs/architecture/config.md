...
`ConfigEnvStorage` (`storage/`) импортируют из-за пределов своего каталога только `dotenv` и тип
`RawConfig` из общих типов конфига: это механика источника, и следующий источник ляжет рядом с
ними. Изоляции от сторон это не гарантирует: `config-container.types.ts` импортирует типы
