...
Форма значений передаётся контейнеру дженериком, и из неё же `Paths` и `ValueByPath`
(`config-container.types.ts`) выводят допустимые пути и тип результата `get()`. Там же
`ConfigPath` и `ConfigValue` — те же типы, применённые к `ConfigValues` (их берёт
`shared/config-value.ts`), и алиас `CC`: зависимость от контейнера везде называется `cc` и
типизируется им.
