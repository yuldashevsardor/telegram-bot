...
Конфигурация живёт в `bootstrap/config/`. `ConfigStorage` (`get(key)`) → `ConfigEnvStorage`
(`dotenv` в конструкторе, читает `process.env`) → `ConfigBuilder.build()` разбирает всё сразу
через `ConfigReader`, проверяет и отдаёт объект `ConfigValues` → `ConfigContainer<ConfigValues>`
его хранит и раздаёт значения и секции методом `get("bot.token")`. Раскрытия `${...}` в `.env`
нет. Тесты подкладывают фейковый сторедж.

`ConfigReader` (`reader/config-reader.ts`) разбирает строки строго: умолчание
...
остаются в `ConfigBuilder` (`builder/config-builder.ts`). Хелперы — публичные методы
отдельного класса, а не приватные методы `ConfigBuilder`: у хелпера может ещё не быть вызова
(так вернулись `getBoolean` и `getArray`), а `noUnusedLocals` не пропускает приватный метод
без вызовов.

`ConfigContainer` (`config-container.ts`) ничего не разбирает и не проверяет: к его созданию
конфигурация уже собрана и провалидирована. Форма значений передаётся ему дженериком, и из
неё же `Paths` и `ValueByPath` (`config-container.types.ts`) выводят допустимые пути и тип
результата `get()`. Там же `ConfigPath` и `ConfigValue` — те же типы, применённые к
`ConfigValues`, их берёт `shared/config-value.ts`.
...
`ConfigEnvStorage` (`storage/`) не импортируют ни одного модуля: это механика источника, и
следующий источник ляжет рядом с ними. `ConfigValues` импортирует типы настроек всех сторон
