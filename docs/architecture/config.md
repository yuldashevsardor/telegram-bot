...
это `CC` — `ConfigContainer<ConfigValues>` с `ConfigValuesBuilder` и `ConfigFileStorage` над
`ConfigEnvStorage` (ниже, «Наблюдение за файлом»), его собирает `ApplicationContext.create()`
([`application.md`](./application.md)). Раскрытия `${...}` ни в `.env`, ни в наблюдаемом файле
нет.
...
`ConfigFileStorage` (`storage/config-file-storage.ts`) — второй источник: читает
`KEY=value`-файл через `dotenv.parse` и кладёт его значения поверх снимка базового источника,
который получает в конструкторе (в приложении это `ConfigEnvStorage`). Поэтому поменять на ходу
можно и то, что задано переменной окружения, — и поэтому же значение из файла перебивает
окружение вместе со всем, что задаёт `docker-compose.app.yml`
...
собственные прошлые значения. Отсутствие файла — не отказ: снимок собирается из одного
окружения, а удаление файла возвращает значения к нему. Нечитаемый путь (нет прав, каталог на
...
Наблюдение объявлено отдельным интерфейсом `WatchableConfigStorage` (`storage/config-storage.ts`,
там же страж `isWatchableConfigStorage`), а не парой методов в `ConfigStorage`: за `process.env`
следить нечем, а vault ([#107](https://github.com/yuldashevsardor/telegram-bot/issues/107))
сообщает об изменениях своим способом. `watch()` опрашивает файл через `fs.watchFile`, а не
подписывается `fs.watch`: приложение работает в контейнере с bind-mount, где inotify-события с
хоста не гарантированы, а сохранение редактора через временный файл с переименованием уводит
inode — вместе с ним `fs.watch` теряет и сам файл. Снимки `stat` сравниваются: слушателя на
отсутствующем файле `watchFile` зовёт сразу после подписки, с нулями в обоих снимках, и этот
вызов изменением не считается — сигнал идёт за правкой содержимого, появлением файла (`mtime` из
нуля) и его удалением (`mtime` в ноль).

Файл попадает в контейнер каталогом: `docker-compose.app.yml` монтирует `./config`, а не сам
файл, потому что одиночный bind-mount файла держится за inode и правка редактора до контейнера
не дошла бы. `.env` так наблюдать нельзя: он в образ не попадает (`.dockerignore`) и приходит
переменными через `env_file`, то есть внутри контейнера файла нет, а его значения уже лежат в
`process.env`. Шаблон наблюдаемого файла — `config/runtime.env.dist`; сам `config/runtime.env` в
`.gitignore` и `.dockerignore`, как `.env`.
...
параллельно: идущая держит промис, а сигналы, пришедшие во время неё, сливаются в один
следующий проход — снимок всё равно читается целиком и увидит последнее состояние источника.
...
срабатывает на правку `limits.common.number`. Упавший слушатель не отменяет рассылку остальным,
его отказ уходит в `onError()`; туда же уходит отказ промиса асинхронного слушателя — по
объявлению слушатель возвращает `void`, но компилятор в такой тип асинхронную функцию
пропускает, и без перехвата её отказ дошёл бы до `unhandledRejection` в `app.ts`
...
([инвариант](./invariants.md)). Наблюдение включается явным `watch()` в конце сборки контекста и
гасится `unwatch()` в `Application.terminate()` — до общего срока остановки и вне него.

| Переменная | Назначение (по умолчанию) |
|---|---|
| `NODE_ENV` | режим приложения (`development`); от него зависят адаптер логгера и порог ([`logging.md`](./logging.md)) |
| `CONFIG_FILE_PATH` | наблюдаемый файл (`<root>/config/runtime.env`); его значения ложатся поверх окружения |
| `CONFIG_FILE_WATCH_INTERVAL` | интервал опроса этого файла, мс (2000), от 0 до 2147483647; `0` — не наблюдать |
