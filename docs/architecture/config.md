...
Интерфейсов два. `ConfigStorage` (`storage/config-storage.ts`) — `load()` и `stop()`: остановка
объявлена у любого источника, потому что освобождать между вызовами `load()` может быть что
угодно — опрос файла, соединение с vault, — и вызывающему не нужно знать, есть ли ей что делать.
`WatchableConfigStorage` (`storage/watchable-config-storage.ts`) добавляет `watch(onChanged)` и
живёт отдельным файлом: за `process.env` следить нечем, а vault
([#107](https://github.com/yuldashevsardor/telegram-bot/issues/107)) сообщает об изменениях своим
способом, так что заглушка `watch()` в каждом источнике была бы лишней. В рантайме интерфейсов
нет, поэтому наблюдаемость проверяется стражем `isWatchableConfigStorage`
(`storage/config-storage.helpers.ts`). Составной источник останавливает и базовый: `stop()` у
`ConfigFileStorage` снимает опрос и зовёт `stop()` того, кого обернул.
...
Путь файла и интервал опроса читаются прямо из `process.env` в `ApplicationContext`, мимо
`ConfigParser`: они нужны, чтобы собрать конфигурацию, а `ConfigParser` работает по снимку,
который из них и получается. Разбор всё равно строгий — недопустимый интервал валит старт
`InvalidConfigError`, а не превращается в умолчание: молча выключенное или ускоренное наблюдение
выглядит как работающее. Верхний предел интервала — `ConfigParser.MAX_TIMER_DELAY`, общий для
всех таймеров Node (почему — таблица ниже и сам `ConfigParser`).
...
параллельно: идущая держит промис в единственном поле состояния, а сигналы, пришедшие во время
неё, помечают в нём же, что нужен ещё один проход, и сливаются в этот один проход — снимок всё
равно читается целиком и увидит последнее состояние источника. Пересборка наружу не вызывается:
единственный её вход — сигнал источника.
...
останавливает `stop()` контейнера — его зовёт `Application.terminate()` до общего срока
остановки и вне него.

| Переменная | Назначение (по умолчанию) |
|---|---|
| `NODE_ENV` | режим приложения (`development`); от него зависят адаптер логгера и порог ([`logging.md`](./logging.md)) |
| `CONFIG_FILE_PATH` | наблюдаемый файл (`<root>/.runtime.env`); его значения ложатся поверх окружения |
| `CONFIG_FILE_WATCH_INTERVAL` | интервал опроса этого файла, мс (2000), целое от 0 до 2147483647; `0` — не наблюдать. Читается мимо `ConfigParser` (выше) |
