...
- **Внешние процессы — только через `ProcessHelper.run()`**, с аргументами массивом.
  ...
  «нормальных» путях `exec` и `execFile` неотличимы. Вызов мимо `ProcessHelper` в репозитории
  один — `spawn` в обёртке `test/mutation-record.ts`: ей нужен живой вывод Stryker и ненулевой
  код как штатный исход ([`testing.md`](./testing.md), «Запись прогона»). Строкой команда
  не собирается и там.
