...
- **Зависимости внедряются только явными `@inject(...)`.** `tsx` (esbuild) не эмитит
  `design:paramtypes`, поэтому inversify не выведет зависимость из типа параметра: у
  класса, который контейнер конструирует сам (`bind().to(...)`), параметр конструктора без
  `@inject` уронит резолв в dev и тестах, а сборка `tsc` метаданные эмитит и ошибку не
  покажет. Логгеры под правило не подпадают: их собирает `ApplicationContext` через `new`,
  а в контейнер они попадают готовыми (`toConstantValue`), inversify их не конструирует.
  Обратная сторона той же асимметрии — параметр, который контейнер заполнять не должен:
  значение конфигурации в умолчании (`configValue`, [`application.md`](./application.md))
  обязано быть помечено `@unmanaged()`, иначе в dev умолчание сработает, а в собранном
  коде inversify сочтёт параметр своей зависимостью.
