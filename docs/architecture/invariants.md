...
- **Зависимости внедряются только явными `@inject(...)`.** `design:paramtypes` не эмитит
  ...
  (`tsconfig.json`). Поэтому inversify не выведет зависимость из типа параметра, и у
  класса, который контейнер конструирует сам (`bind().to(...)`), параметр конструктора без
  `@inject` уронит резолв — одинаково в dev, в тестах и в сборке. Логгеры под правило не
  подпадают: их собирает `ApplicationContext` через `new`, а в контейнер они попадают
  готовыми (`toConstantValue`), inversify их не конструирует.
- **`emitDecoratorMetadata` не включать обратно.** На выключенном держится внедрение
  конфигурации: параметр со значением по умолчанию (`configValue`,
  [`application.md`](./application.md)) остаётся вне зависимостей класса только потому, что
  inversify его не видит. С включённым флагом он увидит все девять таких параметров и
  потребует для них биндинги — и потребует только в сборке, потому что в dev метаданных
  всё равно нет.
