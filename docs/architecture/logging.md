...
`fail()` fallback in `app.ts`, for when the record cannot go through the logger: before the context
has its parts ([`application.md`](./application.md)), and when the write of the logger itself
throws. The rule is held by `no-console: "error"` in `.eslintrc.js`:
