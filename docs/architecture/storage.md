...
слоя под адаптеры хранения нет: у `PgSqlUserRepository` свой каталог со спутником
(`telegram/user/pgsql-repository/`), но внутри подсистемы потребителя.

`User.id` — JS `number` при `bigint` в базе (третья миграция расширила колонку из-под
`int4`); точность до `2^53 - 1`, текущие ID Telegram укладываются. Драйвер без настройки
`types` отдаёт `bigint` строкой, поэтому `UserRow.id` — `string`, а в число его приводит
`PgSqlUserRepository.rowToEntity()`. Параметр `sql<UserRow[]>` — только приведение типа:
с пришедшим компилятор его не сверяет, и пока в `UserRow` стоял `number`, строка молча
доезжала до `User.id`.
