...
stale_claude() {
    ...
    # ведёт сам PR этого дерева, за отставание не принимается. Вместе с ней выпадает и
    # незакоммиченная правка — сверяются деревья двух коммитов, — и это то же исключение:
    # обе сделаны здесь, а проверка ищет отставание от main, а не любое расхождение с ним.
    # Поэтому же влитый в дерево origin/main проверку выключает, пока не уйдёт вперёд, —
    # даже если слияние оставило .claude старым (git merge -s ours): отставания тут нет,
    # предыдущее состояние ветка выбрала сама.
    # Своего fetch тут нет: по несвежему origin/main отставание занижается, но не
    # выдумывается, а ходить в сеть на каждом старте сессии дороже пропущенного расхождения.
