...
# приложения всех деревьев — и поднятые боты, и одноразовые контейнеры разовых целей
# (make migrate/build/test), — а working_dir у контейнера это путь к его дереву.
others=$(
