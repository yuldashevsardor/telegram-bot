...
git fetch --quiet --refmap= origin "+refs/heads/main:$CLEANUP_REF" || die "не удалось получить main с origin — без него нельзя убедиться, что ветка $branch уже влита"
...
# Отказ молчит — единственное такое место в скрипте: терять нечего, несвежая origin/main
# штатна и без всякого отказа (README.md, «Дерево задачи»), а голый `error: cannot lock ref`
# посреди удавшейся уборки читался бы как её поломка.
...
if [ "$listed" -eq 2 ]; then
    ...
    printf "проверить ветку %s на origin не удалось, причина выше.\nРазберитесь с ней сами из основного дерева: cd '%s' && git push origin --delete '%s'\nОтказ «remote ref does not exist» значит, что ветки на origin уже нет: её удаляет GitHub при влитии PR, и с origin убрано всё. Отказ по любой другой причине — скорее всего по той же, что помешала проверке, — про ветку не говорит ничего: повторите команду, когда устраните причину.\n" \
        "$branch" "$main" "$branch" >&2
