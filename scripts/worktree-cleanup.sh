...
# Копируется ссылка, привезённая фетчем выше, а не фетчится своя: второй поход в сеть оставлял
# между собой и первым окно в один round-trip, и мерж, доехавший внутри него, попадал в
# origin/main, но не в $CLEANUP_REF — проверка слитости ниже отказывала ветке, уже влитой. У
# копии окна нет вовсе: обе ссылки заведомо на одном коммите. Объекты привёз тот же fetch, так
# что origin/main встаёт на коммит, который в репозитории уже есть. Ехать она при этом может и
# назад — на мерж, который сосед своим fetch записал сюда, пока уборка шла; это та же
# устарелость, которую README объявляет штатной, и снимает её первый же чужой fetch.
