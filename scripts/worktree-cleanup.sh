...
# последним, обновление общей origin/main — перед проверкой слитости, а повторяется перед
# самим подтягиванием.
set -eu
...
# Ссылка и --refmap= — те же, что у проверки слитости, и по той же причине: комментарий там.
    ...
    # зависит, а после отказавшего дерево следующей задачи ответвлялось бы от точки до
    # влитого PR — той же, от которой ответвлялось убираемое. Сам отказ говорит только про
    # main, так что устаревшую origin/main не назвал бы никто.
