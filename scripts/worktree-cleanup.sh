...
if ! git worktree remove "$root"; then
    # Отказать remove успевает и на полпути: регистрацию он снимает и содержимое сносит раньше,
    # чем спотыкается об остаток, которого не смог удалить, — например, о каталог, созданный в
    # контейнере от чужого uid внутри coverage или reports (они монтируются с хоста,
    # docker-compose.app.yml). Проверка незакоммиченного выше такого остатка не видит: оба
    # каталога в .gitignore.
...
в $main остался на коммите до мержа. Устраните причину и повторите make worktree-cleanup из
этого дерева: повторное гашение уже погашенного приложения безвредно."
    fi
    die "дерево $root удалено не до конца — причина выше: git worktree его уже не знает, а каталог остался.
Погашено приложение дерева вместе с образом и томом. Повторять цель неоткуда — доделайте уборку
руками, начиная с остатка каталога: об него же споткнулся git, поэтому rm может потребовать
сначала разобраться с причиной.
    rm -rf '$root'
    cd '$main'
    git branch -D '$branch'
