...
# слитости, и этой строкой лежат снос образа, удаление дерева и удаление ветки на origin —
# десятки секунд, за которые соседняя сессия успевает влить свой PR. По устаревшей ссылке
# мерж сообщил бы об успехе, оставив main позади.
elif ! git fetch --quiet origin main; then
    printf 'main в %s не подтянут: git fetch origin main отказал, причина выше.\nРазберитесь в основном дереве и повторите там обе команды — уборка дерева задачи при этом уже сделана:\n    git fetch origin main && git merge --ff-only origin/main\n' \
        "$main" >&2
