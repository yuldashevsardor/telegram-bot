...
if [ "$listed" -eq 2 ]; then
    ...
    # добавит. Зато его отказ здесь — штатный исход, а не беда: ветку удаляет GitHub при
    # влитии PR (комментарий выше), — поэтому смысл отказа подсказка договаривает, иначе
    # выполнивший её прочитает «remote ref does not exist» как незаконченную уборку.
    printf "проверить ветку %s на origin не удалось, причина выше.\nРазберитесь с ней сами из основного дерева: cd '%s' && git push origin --delete '%s'\nВетки там может уже не быть: GitHub удаляет её сам при влитии PR, и тогда push откажет — это значит, что уборка полная.\n" \
        "$branch" "$main" "$branch" >&2
