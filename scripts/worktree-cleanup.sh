...
Занятый деревом слот пула токенов держится, пока цел его каталог: снимет слот успешный
повтор, иначе аренда отвалится сама через BOT_TOKEN_TTL (по умолчанию 2 часа)."
    fi
    ...
    # условна и оговорка про слот пула токенов: слот числится за деревом, пока цел каталог
    # ([ -d "$tree" ] в lease_alive, scripts/bot-token.sh), и без каталога говорить не о чем.
...
"
        ...
        slot="
Занятый деревом слот пула токенов держится, пока цел его каталог: снимает слот первая строка
рецепта — иначе аренда отвалится сама через BOT_TOKEN_TTL (по умолчанию 2 часа)."
    fi
