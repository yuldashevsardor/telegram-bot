...
# последним, обновление общей origin/main — перед проверкой слитости, а следом за
# подтянутым main повторяется.
set -eu
...
# Ссылка и --refmap= — те же, что у проверки слитости, и по той же причине: комментарий там.
    ...
    # хвостовой fetch выше привёз тот, до которого main только что подтянут. Без неё
    # origin/main отставала бы от main основного дерева на весь промежуток уборки — тот
    # самый `[ahead N]` в git status -sb, — притом что сети для копии уже не нужно.
    # Отодвинуть ссылку назад копия может шире первой: у той между фетчем и записью нет ни
    # одной команды, а здесь между ними стоит ff-мерж, и чужой fetch, успевший записать в
    # общую ссылку мерж новее внутри этого промежутка, копия перепишет.
    ...
    git update-ref -m "worktree-cleanup: main после ff-мержа" \
