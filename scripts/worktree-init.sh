...
# его создают цели make, — см. комментарий у DC_APP в Makefile. Права как у .env: в файле держат
# значения, которых в окружении нет, вплоть до BOT_TOKEN, значит и секреты те же. Читать его в
# контейнере это не мешает — файловая шара Docker Desktop отдаёт смонтированное владельцу
# процесса, — а вот на Linux-хосте с uid, не совпадающим с node, права придётся ослабить.
