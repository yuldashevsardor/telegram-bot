...
export class FontSignatureMatcher {
    ...
    public constructor() {
        ...
        //
        // Коллекции ("ttcf") здесь нет намеренно, хотя контейнер у неё тот же: в ней
        // несколько шрифтов, а какой из них брать, домен не решает. Движок молча взял бы
        // первый, конверт EOT — отверг бы файл целиком, и одно и то же имя вело бы себя
        // по-разному от пары к паре, поэтому коллекция отсекается на входе и одинаково
        // для всех пар (issue https://github.com/yuldashevsardor/telegram-bot/issues/181).
