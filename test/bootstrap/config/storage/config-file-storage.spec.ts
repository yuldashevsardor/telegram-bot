...
// на единицах обессмыслились бы проверки «сигнала не было»: ожидания в спеке пропорциональны
// интервалу, и `sleep(INTERVAL * 4)` с `sleep(INTERVAL * 6)` свелись бы к единицам миллисекунд.
// Запаса на попадание опроса в такое окно не остаётся, и прошедшая проверка уже не значила бы,
// что опрос в нём был.
const INTERVAL = 25;
...
describe("ConfigFileStorage", () => {
    ...
    // Флаг r+ не обрезает, поэтому содержимое должно быть не короче прежнего: остаток файла
    // запись не убирает, и хвост прежнего значения остался бы в файле. Проверка размера стоит
    // здесь же: сам stat времён файла не меняет и сигнала не даёт, в отличие от обрезки.
    async function change(contents: string): Promise<void> {
        expect(Buffer.byteLength(contents)).to.be.at.least((await fs.stat(filePath)).size);
