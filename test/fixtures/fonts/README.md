...
`/font_generator` и для спеков: сигнатурам нужны настоящие заголовки, а не выдуманные, а
пары конвертируют настоящие шрифты. Какие спеки берут фикстуры —
`grep -rl fixtures test --include='*.spec.ts'`. Каталог лежит в `test/`, а не в `tmp/`,
как раньше: `tmp` перечислен в `.dockerignore` и вдобавок перекрыт томом `app-tmp`,
поэтому файлы оттуда до контейнера не доезжали вовсе.
...
версию и дату сборки. Сверять замену надо сигнатурой формата, а не хешем, и прогоном
`make test`: `font-forge-convertor.spec.ts` конвертирует каждую фикстуру, кроме EOT,
настоящим fontforge во все остальные форматы без EOT, так что замена, которую движок не
откроет, уронит спек при верной сигнатуре.
