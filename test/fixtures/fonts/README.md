...
`grep -rl fixtures test/font-convertor`. Каталог лежит в `test/`, а не в `tmp/`, как
раньше: `tmp` перечислен в `.dockerignore` и вдобавок перекрыт томом `app-tmp`, поэтому
файлы оттуда до контейнера не доезжали вовсе.
